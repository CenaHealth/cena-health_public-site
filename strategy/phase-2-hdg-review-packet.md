...
> **Status update 2026-06-08 (Aaron):** Packet **will NOT be routed to Vanessa** for sign-off. Proceeding **as-approved for design + build progress**. Final partner approvals gate launch only — site cannot ship until they land. The 4 items genuinely requiring partner input (out of 9 in §2 + 3 in technical body) are tracked at [`outstanding-partner-input.md`](outstanding-partner-input.md). Cena-self-decidable items default to HDG-recommended phrasing in §§2-7 below.
>
> **UConn-named upgrade status:** Vanessa replied 2026-06-07: *"UConn said it shouldn't be a problem, but they haven't responded to my red line yet. Let's wait a few more days for them to confirm."* Generic-AMC reference ships in the meantime per IA + voice.md; UConn-named upgrade is a strict-superset edit when clearance lands. Tracked as item #1 on the outstanding-partner-input tracker.

<!-- @Aaron 2026-06-07 inline annotation (preserved): "i don't remember where this thought was left off but i have an outstanding request from you to confirm with Vanessa that UConn gave us permission to mention them on our site. my read is that we can move forward for now. here's the chat contents for your context: Aaron — Drafting public-site copy ...; Vanessa — UConn said it shouldn't be a problem, but they haven't responded to my red line yet. Let's wait a few more days." Resolution: tracker item #1; default-generic ships now. -->
...
<!-- @Aaron 2026-06-07 inline annotation (preserved): "i love that we're checking in on important things like this. feel free to check with Coach but my read on Vanessa is that she won't address this. it's my current opinion that we should move forward as though all of these are approved so we can get a draft built of the site. i will not be launching anything until it's fully approved. i just don't want to block design and build progress on content questions when we can always and will likely have to come back and iterate on content later." Resolution: status block at top — proceed-as-approved for design+build; launch-gate-only for partner approvals. -->
...
Nine claims originally needed explicit decision. **Per 2026-06-08 status update at top of doc**, 8 of 9 are Cena-self-decided defaulting to HDG-recommended phrasing; 1 is tracked at [`outstanding-partner-input.md`](outstanding-partner-input.md) as a launch-gating item.

<!-- @Aaron 2026-06-07 inline annotation (preserved): "i think we already do this reflexively for all projects 🤞 but just in case, please make sure we're tracking all outstanding items like this for the project. i want to move forward but i don't want to lose these needs which will have to remain outstanding for the moment." Resolution: outstanding-partner-input.md created as the canonical tracker. -->

| # | Claim | Where it ships | Recommended phrasing | Cena decision |
|---|---|---|---|---|
| 1 | The pilot-reference language ("a live academic-medical-center HIV-nutrition program runs on this blueprint today") | `/reference-program` headline + homepage proof strip | Use generic-AMC language at launch; upgrade to "UConn Health's" only after you confirm UConn comms has cleared the named reference | **Generic-AMC: APPROVED (Cena). UConn-named upgrade: TRACKED** ([item #1](outstanding-partner-input.md)) — awaiting UConn red-line response via Vanessa |
| 2 | "BAA-ready" as a public posture claim | Homepage proof strip · `/how-the-substrate-works` security section · `/for-clinical-leaders` proof | Ship as architectural claim — *"BAA-ready architecture"* (about how Cena is built). Do NOT ship "BAAs in place" anywhere (that's partner-by-partner and changes per deal) | **APPROVED (Cena)** — ship architectural claim as recommended |
| 3 | Naming our cloud / vendor stack publicly (Google Cloud, Vertex AI, etc.) | `/how-the-substrate-works/integration` sub-page · NDA-room landing teaser | My recommendation: **don't name vendors publicly**; mention "HIPAA-eligible cloud infrastructure with executed BAAs across the vendor stack" and route specifics to NDA room. Reason: HITRUST-aligned health systems want it; sovereignty-sensitive payers don't; naming raises questions we don't need at recognition stage. Your call. | **APPROVED (Cena)** — don't name vendors publicly; categories-only in teaser; specifics to NDA room |
| 4 | Partner-clinician accountability framing | `/how-the-substrate-works/provenance-and-accountability` · `/for-clinical-leaders` thesis · `About` page | Ship as: *"During pilots, partner-institution clinicians retain clinical accountability for every recommendation Cena's substrate generates. Cena's role is the audit-grade trail that makes that accountability defensible."* This is the model you've been holding the line on internally; this is its public form. | **APPROVED (Cena)** — ship as recommended; matches accountability model Cena has been holding |
| 5 | NDA-gated documentation room — what's in vs. out of the public-facing teaser | `/how-the-substrate-works` hub CTA · NDA-room landing page | Public teaser names *categories* of what's in the room (architecture diagrams, security control matrices, BAA template, vendor stack disclosures, audit-log schema). Does NOT show the artifacts themselves. Room access requires: name, role, partner-org, BAA-counsel contact. | **APPROVED (Cena)** — categories-only public; artifacts gated |
| 6 | Sample audit trail — synthetic-data discipline | `/how-the-substrate-works/sample-audit-trail` | Page must carry a top-of-page label: *"All sample data on this page is synthetic. No patient referent, no partner referent."* Every annotation field must be reviewable as accurate-to-what-the-system-actually-does. | **APPROVED (Cena)** — synthetic-data label canonical |
| 7 | "Tenant-isolated" / "per-program data partition" claim | `/how-the-substrate-works` security section · Hard-claim Appendix C #3 | Ship as: *"Each partner program runs in its own data partition; each role on a care team sees what their job requires, enforced by the system."* Verifiable in [`ava.md`](../../../Knowledge/Areas/Meta/Entities/cena-health/ava.md) vendor-independence + role-scoped-PHI architecture. Safe. | **APPROVED (Cena)** — verifiable against ava.md |
| 8 | FHIR / Epic integration language | `/how-the-substrate-works/integration` sub-page · `/for-clinical-leaders` how-it-works cards | Ship as: *"Built to connect to the systems partners already run — FHIR endpoints, file drops, API conventions partners specify."* Do NOT claim "Epic-certified," "approved Epic partner," or "live Athena API" without primary-source verification (per the Accenture-app-flagged claim 2026-05-26). | **APPROVED (Cena)** — generic integration language ships; specific Epic-/Athena-named claims do NOT ship without partner-source verification (would join the tracker if attempted) |
| 9 | Forward-looking-disclosure preference | `/proof` empty-slot framing · NDA-room landing | When does Cena upgrade public framing from "BAA-architected" to "BAAs in place across N partners"? My recommendation: hold "BAA-architected" public until at least 2 BAAs are executed; first BAA gets named only with partner comms clearance. Your call on whether 2 is the right threshold. | **APPROVED (Cena)** — hold "BAA-architected" public until 2 BAAs executed; revisit at threshold |

~~**Reply shape that lands fastest for you:** row-by-row, one line per. *"1 approve. 2 approve. 3 call. 4 approve with edit: [...]"* etc. Or grab Aaron synchronously and walk it. If you call, his copy of the packet is open.~~ — *Vanessa-facing reply guidance retired per 2026-06-08 status update; packet not routed to her.*
